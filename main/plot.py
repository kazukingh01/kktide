...
font_path   = './ipaexg.ttf'  # ここにipaexg.ttfの正確なパスを指定
...
if __name__ == "__main__":
    ...
    if args.list:
        ...
        df_gd["prev"]  = df_gd.groupby("symbol", group_keys=False)["level"].apply(lambda x: pd.DataFrame([float("nan")] + x[:-1].tolist(), index=x.index))
        df_sd["prev"]  = df_sd.groupby("symbol", group_keys=False)["level"].apply(lambda x: pd.DataFrame([float("nan")] + x[:-1].tolist(), index=x.index))
        df_gd["level"] = df_gd["level"] - df_gd["prev"]
        df_sd["level"] = df_sd["level"] - df_sd["prev"]
        df_d           = df_gd[["datetime", "symbol", "highlow"]].copy()
        df_d["level"]  = df_gd["level"] - df_sd["level"]
        fig, ax   = plt.subplots(int(len(symbols) ** (1/2)), math.ceil(len(symbols) ** (1/2)), figsize=(15, 9))
        ...
        for df, str_gs in zip([df_g, df_s, df_gd, df_sd, df_d], ["genbo", "suisan", "genbo_diff", "suisan_diff", "diff"]):
            for (symbol, highlow), dfwk in df.groupby(["symbol", "highlow"]):
                if highlow == 0: ax[symbol].plot(   dfwk.loc[~dfwk["level"].isna(), "datetime"], dfwk.loc[~dfwk["level"].isna(), "level"], color=colors[symbol], linestyle={"genbo": "--", "suisan": ":", "genbo_diff": "--", "suisan_diff": ":", "diff": "-"}[str_gs], label=f"{str_gs}: {symbol} {dict_mst_s[symbol]}")
                if highlow == 1: ax[symbol].scatter(dfwk.loc[~dfwk["level"].isna(), "datetime"], dfwk.loc[~dfwk["level"].isna(), "level"], color=colors[symbol], marker={"genbo": "*", "suisan": "^"}[str_gs])
                if highlow == 2: ax[symbol].scatter(dfwk.loc[~dfwk["level"].isna(), "datetime"], dfwk.loc[~dfwk["level"].isna(), "level"], color=colors[symbol], marker={"genbo": "o", "suisan": "v"}[str_gs])
